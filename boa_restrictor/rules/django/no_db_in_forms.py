...
class NoDjangoDbImportInFormsRule(Rule):
    """
    Ensures that no Django low-level database functionality is imported and therefore used in the form layer.
    """

    RULE_ID = f"{DJANGO_LINTING_RULE_PREFIX}010"
    RULE_LABEL = 'Do not use "django.db" in the form layer. Move it to a manager instead.'

    def check(self) -> list[Occurrence]:
        occurrences = []

        if not is_layer_file(self.file_path, layer="forms"):
            return occurrences
